...
epsilon <- pd$read_pickle("E:/MS_Thesis/publication_stuff/refactored_code/diffpriv_R/epsilon.pkl")


log_errors <- function(dataset_folder_path, query, i, eps) {
  # mean error
  err <- read.csv(paste0(dataset_folder_path,"/diffpriv_micro/",query,"/results_dataset_",i,"/error/DP_error_eps_",eps,".csv"))
  mean_err <- mean(err$X)
  write.table(mean_err, paste0(dataset_folder_path,"/diffpriv_micro/",query,"/results_dataset_",i,"/mean_error/mean_error.csv"), append = T, row.names = F, col.names=F, eol = "\n")
  # mean scaled error
  scaled_err <- read.csv(paste0(dataset_folder_path,"/diffpriv_micro/",query,"/results_dataset_",i,"/scaled_error/DP_scaled_error_eps_",eps,".csv"))
  mean_scaled_err <- mean(scaled_err$X)
  write.table(mean_scaled_err, paste0(dataset_folder_path,"/diffpriv_micro/",query,"/results_dataset_",i,"/mean_scaled_error/mean_scaled_error.csv"), append = T, row.names = F, col.names=F, eol = "\n")
  # std error
  std_err <- sd(err$X)
  write.table(std_err, paste0(dataset_folder_path,"/diffpriv_micro/",query,"/results_dataset_",i,"/std_error/std_error.csv"), append = T, row.names = F, col.names=F, eol = "\n")
  # std scaled error
  std_scaled_err <- sd(scaled_err$X)
  write.table(std_scaled_err, paste0(dataset_folder_path,"/diffpriv_micro/",query,"/results_dataset_",i,"/std_scaled_error/std_scaled_error.csv"), append = T, row.names = F, col.names=F, eol = "\n")
  # time consumption
  t <- read.csv(paste0(dataset_folder_path,"/diffpriv_micro/",query,"/results_dataset_",i,"/time/time_eps_",eps,".csv"))
  time_taken <- mean(t$X)
  write.table(time_taken, paste0(dataset_folder_path,"/diffpriv_micro/",query,"/results_dataset_",i,"/execution_time.csv"), append = T, row.names = F, col.names=F, eol = "\n")
