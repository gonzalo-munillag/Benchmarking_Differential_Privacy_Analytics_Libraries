...
# ## Goal: We would like to create different arrays that represent different characteristic combinations in terms of scale (number of elements, sample size), domain (the r>
# ## Levels:
# ### Scale: 1000, 10000, 100000
# ### Domain: std= 50, std=250, std=500
# ### Shape: skew = 0, skew = 5, skew = 50
# ## We define three levels for each of these terms, yielding 27 possible combinations. We will later on apply differencial privacy mechanism to the arrays to test how the># ## This notebook only shows how to tune each of the parameters of a normal distribution created with scipy
...
def dataset_generation(a, scale, size, f):
    ...
    for i in data:
        guestFile = open("dataset_{}.csv".format(f),"a")
        guestFile.write(str ("%.4f"% (i)))
        guestFile.write("\n")
        guestFile.close()

a = [[5],[250],[1000,10000,100000,100000000]]
