...
for i in range(number_of_csv_files):
    ...
    for eps in epsilon:
        ...
        print('(Smartnoise)  Mean Query for Epsilon = {e} mean time is = {t} and mean memory is = {m}'.format(e = eps, t=time_list, m=memory_list))



        df_time = pd.DataFrame(time_list)
        df_memory = pd.DataFrame(memory_list)
        df_time.to_csv('~/publication/bigO/results/smartnoise/mean_time_eps_{eps}_dataset_{i}.csv'.format(eps=eps, i=i), index = False)
        df_memory.to_csv('~/publication/bigO/results/smartnoise/mean_mem_eps_{eps}_dataset_{i}.csv'.format(eps=eps, i=i), index = False)
