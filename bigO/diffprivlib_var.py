...
for i in range(number_of_csv_files):
    ...
    for eps in epsilon:
        ...
        for itr in range(number_of_experiments):
            ...
            DP_result = dpt.var(normal_distribution, eps, bounds=(minimum, maximum))
            time_list.append(time.time() - begin_time) # running time
            memory_list.append(process.memory_info().rss) # memory consumption
        print('Dataset for Epsilon = {e} mean time is = {t} and mean memory is = {m}'.format(e=eps, t=time_list, m=memory_list))


        df_time = pd.DataFrame(time_list)
        df_memory = pd.DataFrame(memory_list)
        df_time.to_csv('~/publication/bigO/results/diffprivlib/var_time_eps_{eps}_dataset_{i}.csv'.format(eps=eps, i=i), index = False)
        df_memory.to_csv('~/publication/bigO/results/diffprivlib/var_mem_eps_{eps}_dataset_{i}.csv'.format(eps=eps, i=i), index = False)
