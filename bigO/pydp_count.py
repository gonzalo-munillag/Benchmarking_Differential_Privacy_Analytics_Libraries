...
for i in range(number_of_csv_files):
    ...
    for eps in epsilon:
        ...
        for itr in range(number_of_experiments):
            ...
            x = dp.Count(epsilon=eps, dtype='float') 
        ...
        df_time.to_csv('~/publication/bigO/results/pydp/count_time_eps_{eps}_dataset_{i}.csv'.format(eps=eps, i=i), index = False)
        df_memory.to_csv('~/publication/bigO/results/pydp/count_mem_eps_{eps}_dataset_{i}.csv'.format(eps=eps, i=i), index = False)
